...
class GoogleWeatherAPI(object):
  ...
  def FarenheightToCelsius(self, temperature_f):
  ...
  def GetDataFromRegion(self, region='San Francisco'):
    ...
    data_region['temp_c'] = self.FarenheightToCelsius(data_region['temp_f'])
    ...
    data_region['humidity'] = float(soup.find("span", attrs={"id": "wob_hm"}).text.replace('%', ''))  # % Relative Himidity
